...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    // Parse request
    const { account_id, user_query, filters = {}, debug }: ContextRequest = req.body;
    ...
    const [campaignsResult, adGroupsResult, keywordsResult] = await Promise.allSettled([
      fetch(`${process.env.VERCEL_URL || 'http://localhost:3000'}/api/google-ads/campaigns`, {
        method: 'POST',
        headers: {
      ...
      fetch(`${process.env.VERCEL_URL || 'http://localhost:3000'}/api/google-ads/adgroups`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      fetch(`${process.env.VERCEL_URL || 'http://localhost:3000'}/api/google-ads/keywords`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
    ...
    // Process results
    const campaigns = campaignsResult.status === 'fulfilled' && campaignsResult.value.ok ? 
      (await campaignsResult.value.json()).campaigns : [];
    const adGroups = adGroupsResult.status === 'fulfilled' && adGroupsResult.value.ok ? 
      (await adGroupsResult.value.json()).adGroups : [];
    const keywords = keywordsResult.status === 'fulfilled' && keywordsResult.value.ok ? 
      (await keywordsResult.value.json()).keywords : [];
    ...
    if (clicks > 0) {
      ...
      if (conversionRate < 0.01 && cost > 50) {
        recommendations.push({
          type: 'keyword',
          priority: 'medium',
          title: `Optimize keyword: ${keyword.keyword?.text}`,
          ...
          affected_entity: keyword.keyword?.text
        });
      }
    }
  });
